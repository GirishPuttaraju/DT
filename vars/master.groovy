...
def call() {
   ...
   //env.HIPCHAT_ROOM = "CI-CD-FGLFRN_StoreReceiving_AX_Publisher"
    ...
    def targetGroup = conf.targetGroup_PROD         // Branch type: conf.bwTargetGroup = dev, conf.bwTargetGroup_QA = qa
    def targetHost = conf.targetHost_PROD           // Host to deploy to: conf.bwTargetHost = BW Dev, conf.bwTargetHost_QA = BW QA
    def targetHostUser = "svc.cicdbigdata"                           // SSH user: change to your service user
    ...
    try {
        ...
        stage('Checkout Profile Configuration') {
            ...
            stash name:'prodvars', includes:"${conf.mvnProfileRepo}/*"
        ...
        stage('Tibco EMS') {
            ...
            cmd = "atf-cli ems-provision-resources --map pipeline/master/resource-setup_prod.yml --ems-alias ems_prod_ssl"
        ...
        stage('Tibco BW Put File') {
            ...
            cmd = "atf-cli tibco-put-file --bw-alias bw_prod --input-file ${bw_file_path}"
            atf.runCommand(sshKeyId, targetHost, targetHostUser, env.GIT_REPO, cmd)
        }*/
      
       stage('Tibco BW Check') {
           echo pipelineConfig.pad('Check BW variables')
           cmd = "atf-cli tibco-bw-check --bw-alias bw_prod --input-file ${pomFilePathName.replace('.parent','')}"
           atf.runCommand(sshKeyId, targetHost, targetHostUser, env.GIT_REPO, cmd)
       }
        ...
        docker.image('artifactory.corp.ad.ctc:5000/datatech/immutable/maven-jenkins-agent').inside() {
            stage('Build and Deploy BW Artifacts') {
                ...
                for (file in findFiles(glob: "${profileBaseDir}/**/${profile}")) {
                    sh "source ./tibcoVenv/bin/activate; python ${conf.mvnProfileRepo}/substvar.py ${file.path} ${conf.mvnProfileRepo}/${conf.mvnProdVarsFile} prod"
                }
                echo pipelineConfig.pad("Run Maven")
                mvn.run(pomFilePathName, profile, mvnGoals, conf, conf.bwTargetHost_PROD_AJ)
            }
        }
        ...
        stage('Project cleanup and rollback if failed') {
            ...
            rollbackIfFailed(sshKeyId, targetHost, targetHostUser, env.GIT_REPO, "source ./pipeline/master/rollback_prod.sh")

            /* Uncomment to activate resources cleanup
                echo pipelineConfig.pad("Resources cleanup")
                cmd = "atf-cli sys-provision-resources --map pipeline/master/resource-cleanup_prod.yml --hive-alias bw_prod"
                atf.runCommand(sshKeyId, targetHost, targetHostUser, env.GIT_REPO, cmd)
                */
