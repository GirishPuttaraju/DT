...
def call() {
   ...
   //env.HIPCHAT_ROOM = "CI-CD-FGLFRN_StoreReceiving_AX_Publisher"
    ...
    def sshKeyId = ""
    def branchName = env.GIT_BRANCH.replaceAll('preprod/', '')
    ...
    try {
        ...
        stage('Checkout Profile Configuration') {
            ...
            stash name:'ppvars', includes:"${conf.mvnProfileRepo}/*"
        ...
        stage('Tibco EMS') {
            ...
            cmd = "atf-cli ems-provision-resources --map pipeline/preprod/${branchName}/resource-setup_pp.yml --ems-alias ems_pp_ssl"
        ...
        stage('Tibco BW Put File') {
            ...
            cmd = "atf-cli tibco-put-file --bw-alias bw_preprod --input-file ${bw_file_path}"
        ...
        stage('Tibco BW Check') {
           ...
           cmd = "atf-cli tibco-bw-check --bw-alias bw_preprod --input-file ${pomFilePathName.replace('.parent','')}"
           atf.runCommand(sshKeyId, targetHost, targetHostUser, env.GIT_REPO, cmd)
       }
        ...
        docker.image('artifactory.corp.ad.ctc:5000/datatech/immutable/maven-jenkins-agent').inside() {
            stage('Build and Deploy BW Artifacts') {
                ...
                for (file in findFiles(glob: "${profileBaseDir}/**/${profile}")) {
                    sh "source ./tibcoVenv/bin/activate; python ${conf.mvnProfileRepo}/substvar.py ${file.path} ${conf.mvnProfileRepo}/${conf.mvnPpVarsFile} pp"
                }
                echo pipelineConfig.pad("Run Maven")
                mvn.run(pomFilePathName, profile, mvnGoals, conf, conf.bwTargetHost_PREPROD_AJ)
            }
        }
        ...
        //         def profileBaseDir = mvnProfile.truncatePathName(pomFilePathName, 1, 2, '/')
        //         for (file in findFiles(glob: "${profileBaseDir}/*${profile}")) {
        //             sh "source ./tibcoVenv/bin/activate; python ${conf.mvnProfileRepo}/substvar.py ${file.path} ${conf.mvnProfileRepo}/${conf.mvnPpVarsFile} qa"
        //         }
        //         echo pipelineConfig.pad("Run Maven")
        //         mvn.run(pomFilePathName, profile, mvnGoals, conf, conf.bwTargetHost_PREPROD_AJ)
        //     }
        ...
        stage('Project cleanup and rollback if failed') {
            ...
            /* Uncomment to activate resources cleanup
                ...
                cmd = "atf-cli sys-provision-resources --map pipeline/master/resource-cleanup_pp.yml --hive-alias bw_preprod"
                atf.runCommand(sshKeyId, targetHost, targetHostUser, env.GIT_REPO, cmd)
                */
